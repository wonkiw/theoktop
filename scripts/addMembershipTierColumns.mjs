...
async function main() {
  ...
  await pool.query(`
    ALTER TABLE users
    ADD COLUMN IF NOT EXISTS membership_tier VARCHAR(20) NOT NULL DEFAULT 'general',
    ADD COLUMN IF NOT EXISTS premium_since TIMESTAMP,
    ADD COLUMN IF NOT EXISTS premium_upgraded_by VARCHAR(255)
  `)
  console.log('\n컬럼 추가 완료')
  ...
  try {
    await pool.query(`
      ...
      CHECK (membership_tier IN ('general', 'premium'))
    `)
